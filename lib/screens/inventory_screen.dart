...
// Ожидается, что в таблице public.user_credentials для пользователя есть колонка
// `inventory` (json/jsonb или text), содержащая либо объект вида
//   { "sword": 3, "shield": 1 }
// либо массив объектов вида
//   [ { "name": "sword", "count": 3 }, { "name": "shield", "count": 1 } ]
// В коде обработаны оба формата, а также возможные варианты, где числа представлены строками.
import 'dart:convert';
...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  bool _loading = false;
  String? _error;
  Map<String, int> _items = {}; // itemName -> count
  ...
  Future<void> _loadInventory() async {
    ...
    try {
      ...
      final parsed = _parseInventory(inv);
      setState(() {
        _items = parsed;
      });
  ...
  // Парсер разных вариантов JSON-структуры inventory -> Map<String,int>
  Map<String, int> _parseInventory(dynamic inv) {
    ...
    // Если это объект/Map: ключи — имена предметов, значения — числа
    if (decoded is Map) {
    ...
    if (decoded is List) {
      for (final el in decoded) {
        if (el is Map) {
          final name = (el['name'] ?? el['item'] ?? el['title'] ?? '').toString();
